-- ================================================
-- UPGRADE DE AUDITORÍA SIMPLE A COMPLETA
-- ================================================
-- Este script actualiza el sistema de auditoría simple al completo

-- Primero, eliminamos las funciones existentes que van a cambiar
DROP FUNCTION IF EXISTS audit.get_record_history(text,text,integer);
DROP FUNCTION IF EXISTS audit.get_audit_stats();
DROP FUNCTION IF EXISTS audit.cleanup_old_records(integer);

-- Ahora ejecutamos las nuevas tablas que no existían en el simple
...
-- Actualizar la tabla audit_log para agregar la foreign key a empleados
ALTER TABLE audit.audit_log 
ADD COLUMN IF NOT EXISTS empleado_id UUID REFERENCES public.empleados(id);

-- Actualizar el trigger para incluir empleado_id
...
-- Ahora creamos las funciones nuevas (completas)
...
BEGIN
    ...
    SELECT 
        ...
        al.operacion,
        COALESCE(e.nombre, 'Sistema') as empleado_nombre,
        al.datos_anteriores,
        al.datos_nuevos,
        ...
        SELECT 
            ...
            COALESCE(
                (SELECT COUNT(*) 
                 ...
                 JOIN audit.audit_sessions ses ON aa.sesion_id = ses.sesion_id
                 WHERE ses.empleado_id = al.empleado_id
                   AND aa.resultado = 'denegado'
                   AND aa.timestamp_acceso > NOW() - (horas_atras || ' hours')::INTERVAL), 
                0
            ) as accesos_fallidos
        FROM audit.audit_log al
...
BEGIN
    ...
    -- Solo insertar si tenemos sesión activa
    IF current_session_id IS NOT NULL AND current_session_id != '' THEN
...
CREATE POLICY audit_sessions_employee_access ON audit.audit_sessions
...
CREATE POLICY audit_sessions_admin_access ON audit.audit_sessions
...
-- Políticas similares para audit_access
CREATE POLICY audit_access_admin_access ON audit.audit_access
    FOR ALL TO authenticated
    USING (
        EXISTS (
            SELECT 1 FROM public.empleados e
...
-- Notificación de upgrade exitoso
DO $$
BEGIN
    RAISE NOTICE 'Upgrade de auditoría simple a completa realizado exitosamente';
    RAISE NOTICE 'Nuevas tablas: audit_sessions, audit_access, audit_errors';
    RAISE NOTICE 'Funciones actualizadas: get_record_history (ahora incluye empleado_nombre)';
    RAISE NOTICE 'Nuevas funciones: get_employee_activity, detect_suspicious_activity, start_session, end_session, log_module_access';
    RAISE NOTICE 'Sistema de auditoría completo ahora disponible';
END $$;
