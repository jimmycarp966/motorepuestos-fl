-- Script robusto para configurar el sistema de cuenta corriente
-- Ejecutar en el SQL Editor de Supabase
-- Este script maneja mejor los conflictos con funciones existentes
...
-- 4. Eliminar funciones existentes de forma segura
DO $$
BEGIN
    -- Eliminar validar_limite_credito si existe
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'validar_limite_credito') THEN
    ...
    -- Eliminar actualizar_saldo_cuenta_corriente si existe
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'actualizar_saldo_cuenta_corriente') THEN
